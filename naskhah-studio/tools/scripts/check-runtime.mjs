...
const checks = [
  ...
  ['Legacy login module does not bind duplicate handlers', login.includes('Authentication form ownership lives in js/core/bootstrap.js') && !login.includes("addEventListener('submit'")],
  ['Secure login Edge Function usage', bootstrap.includes('/functions/v1/naskhah-secure-login')],
  ['Secure login calls setSession', bootstrap.includes('await setSession(')],
  ['User suspension guard preserved', bootstrap.includes("state.profile?.status==='suspended'")],
  ['Admin role guard preserved', bootstrap.includes("state.profile?.role!=='admin'")],
  ['Turnstile token required', bootstrap.includes('cf-turnstile-response')],
  ['Recovery uses direct Supabase auth', bootstrap.includes('sb.auth.resetPasswordForEmail')],
  ['Password recovery event handled', bootstrap.includes("event!=='PASSWORD_RECOVERY'")],
